...
const SingleStat = props => {
  ...
  return (
    <StatPanel>
      ...
      {!noValue && (
        <Text style={styles.value}>{props.value.toLocaleString()}</Text>
      )}
      {noValue && !props.isLoading && (
        <Text style={styles.noReports}>No reports 🎉</Text>
      )}
      {props.isLoading && (
        <ActivityIndicator size="large" style={styles.values} />
      )}
